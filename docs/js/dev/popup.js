...
function initSliders() {
  ...
  function updateHeroGalleryImages(swiper) {
    swiper.slides.forEach((slide2) => {
      const img = slide2.querySelector(".hero-gallery__image img");
      if (!img) return;
      const progress = clamp(slide2.progress, -1, 1);
      const absProgress = Math.abs(progress);
      const translateX = progress * 30;
      const scale = 1 + absProgress * 0.6;
      img.style.transform = `translateX(${translateX}%) scale(${scale})`;
      const rawProgress = Math.abs(slide2.progress);
      let opacity = 1;
      if (rawProgress > 1) {
  ...
  function setHeroGalleryImagesTransition(swiper, duration) {
    swiper.slides.forEach((slide2) => {
      ...
      slide2.style.transitionDuration = `${duration}ms`;
    });
  }
  ...
  filterBlocks.forEach((filterBlock) => {
    ...
    function applyFilters() {
      ...
      slides.forEach((slide2) => {
        const tags = slide2.dataset.refTags.split(",").map((t) => t.trim());
        const match = selectedFilters.some((f) => tags.includes(f));
        slide2.style.display = match ? "" : "none";
      });
      swiper.updateSlides();
      swiper.update();
    }
...
function initTicker() {
  ...
  tickers.forEach((ticker) => {
    ...
    if ("ResizeObserver" in window) {
      ...
      const onResize2 = () => applyState();
      window.addEventListener("resize", onResize2, { passive: true });
      ticker._tickerResizeFallback = onResize2;
    }
